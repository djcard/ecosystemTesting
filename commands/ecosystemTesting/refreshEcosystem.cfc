component {
	...
	function run(
		...
		if(evergreenBranches) {
			var parentData = obtainParents(sites);
		}
		...
		repoData.each( function( item ){
			command( "ecosystemTesting refreshSite projectname=#projectname# sitename=#item# branch=#repoData[ item ]# parentBranch=#parentData[ item ]# evergreenBranches=#evergreenBranches#" ).run();
		} );
	...
	function obtainParents( sites ){
		var repoData = {};

		sites.each( function( item ){
			var branch       = ask( message = "What is the full name of the parent to use for #item# ? : " );
			repoData[ item ] = branch;
		} );
		return repoData;
	}
