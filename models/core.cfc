component {
    ...
    function checkModuleKeyExists() {
        ...
        if (!settings.keyExists('modules')) {
            try {
                base.command('config set modules.ecosystemTesting=true').run();
            } catch (any err) {
            };
        }
        else  if(!settings.modules.keyExists('ecosystemTesting')){
            try {
                base.command('config set modules.ecosystemTesting={}').run();
            } catch (any err) {
            };
        }
    }
    ...
    function obtainProjectData(required string projectName) {
        if (!settings.modules.keyExists('ecosystemTesting') || !settings.modules.ecosystemTesting.keyExists(projectName)) {
            print.line('The #projectName# project doesn''t exist yet!').toConsole();
            return {};
        }
    ...
    function clearFolder(folderpath) {
        if (server.os.name == 'Linux') {
            try {
                base.command('!#commandPrefix()# rm -r #folderpath#/*').run();
            } catch (any err) {
                print.line('Could not delete Folder');
            }
            try {
                base.command('!#commandPrefix()# rm -r #folderpath#/.[!.]* ..?*').run();
            } catch (any err) {
                print.line('Could not delete files with leading .');
            }
        } else {
    ...
    function cloneSite(required string sitename, required string projectName) {
        ...
        base.command('!#commandPrefix()# git clone  #createDomainLogin(sitedata.repoSite, data.username, data.pat)##siteData.reponame#.git #arguments.folder#')
            .run();
    }
    ...
    function cloneBranch(
        ...
        print
            .line(' Running: ''!#commandPrefix()# git clone  --branch #branch# #createDomainLogin(arguments.repoSite, data.username, data.pat)##repo#.git #arguments.folder#''')
            .toConsole();
    ...
    function obtainFiles(projectname, sitename) {
        ...
        siteData.fileNames.each(function(item) {
            ...
            try {
                print.line('Copying #fromFileName# to #toFileName#').toConsole();
    ...
    function obtainDockerContainers() {
        var allDockerSites = base
            .command('!#commandPrefix()# docker')
            .params('ps --all --no-trunc --format=''{{json .}}''')
            .run(returnOutput = true);
        var allContainers = allDockerSites
            .listToArray(chr(10))
            .map(function(item) {
    ...
    function restartContainer(containerName = '') {
        if (arguments.containerName.len()) {
            try {
                base.command('! #commandPrefix()# docker')
                    .params('restart #containerName#')
                    .run();
            } catch (any err) {
    ...
    function reBuildImage(containerName = '') {
        if (arguments.containerName.len()) {
            try {
                base.command('! #commandPrefix()# docker-compose')
                    .params('build #containerName#')
                    .run();
            } catch (any err) {
                print.line('Could not build #containerName#. Continuing').toConsole();
            }
        }
    }
